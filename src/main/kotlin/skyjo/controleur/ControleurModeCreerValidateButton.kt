...
class ControleurModeCreerValidateButton(val vue: VueGeneral, val modele: Partie) : EventHandler<ActionEvent> {
    private val scope = CoroutineScope(Dispatchers.IO)


    override fun handle(event: ActionEvent) {
        val nbPlayer: Int = vue.vueSelectedModeModaleCreer.getSpinner().value

        scope.launch {

            withContext(Dispatchers.Default) {
                modele.commencerPartie(nbreJoueur = nbPlayer)
            }

            while (isActive) {
                delay(800)
                withContext(Dispatchers.IO) {
                    modele.recupEtapePartie()
                }
